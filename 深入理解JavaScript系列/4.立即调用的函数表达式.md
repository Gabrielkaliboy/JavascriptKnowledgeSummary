...
### 3.问题核心
当你声明类似function foo(){}或var foo = function(){}函数的时候，在后面加个括弧就可以实现自执行，例如foo()，看代码：
```javascript
...
//
```
上述代码，如果运行，第2个代码会出错，因为在解析器解析全局的函数或者函数内部function关键字的时候，默认是函数声明，而不是函数表达式，如果你不明确的告诉浏览器，它默认声明成一个缺少名字的函数(匿名函数)，并且抛出一个语法错误信息，因为函数声明需要一个名字。
...
### 4.旁白：函数(function)，括弧(paren)，语法错误(SyntaxError)
...
### 5.自执行函数表达式
要解决上述问题，非常简单，我们只需要用大括弧将代码全部括住就行了，因为JavaScript里括弧()里面不能包含语句，所以在这一点上，解析器在解析function关键字的时候，会将相应的代码解析成function表达式，而不是function声明。
...
上面所说的括弧是消除歧义的，其实压根就没必要，因为括弧内部期望的就是函数表达式，但是我们依然用它，主要是为了方便阅读，当你让这些已经自动执行的表达式赋值给一个变量的时候，我们看到开头有括弧(，很快就能明白，而不需要将代码拉到最后看看到底有没有加括弧。
...
<script>
    ...
    for(var i=0;i<elems.length;i++){
        (function(lockIndex){
            elems[i].addEventListener('click',function(e){
                e.preventDefault();
                alert("我是第"+lockIndex+"链接");//这样就会弹出对应的
                //alert("我是第"+i+"链接");如果这么写还是弹出总个数
            },"false");
        })(i)
    }
</script>
...
```
其实，上面2个例子里的lockIndex变量，也可以换成i，因为和外面的i不在一个作用于，所以不会出现问题，这也是匿名函数+闭包的威力。


### 7.自执行匿名函数和立即执行的函数表达式区别
...
3. [Named function expressions](http://kangax.github.io/nfe/) - Juriy “kangax” Zaytsev

4. [全面解析Module模式- Ben Cherry](http://www.cnblogs.com/TomXu/archive/2011/12/30/2288372.html)（大叔翻译整理）
